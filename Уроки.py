...
for x in range(100, 1000):
    if (int(str(x)[0]) + int(str(x)[1]) + int(str(x)[2])) == N:
        print(x)

# 3) Найти количество всех трехзначных чисел, средняя цифра которых равна сумме первой и второй цифр.
# ! Наверное имеется ввиду и третьей цифры

summ = 0
for x in range(100, 1000):
    if int(str(x)[0]) + int(str(x)[2]) == int(str(x)[1]):
        summ += 1
print(summ)
#   
...
for x in range(100, 1000):
    if factorial(int(str(x)[0])) + factorial(int(str(x)[1])) + factorial(int(str(x)[2])) == x:
        print(x)
